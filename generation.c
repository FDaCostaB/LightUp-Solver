...
int nbCasesLibresAdj(Grille *grille,int i){
    int n = 0;
    if(i-TAILLE>=0 && ( grille->tab[i-TAILLE] == LIBRE || grille->tab[i-TAILLE] ==INCONNU) ){
        n ++;
    }
    if((i-1)%TAILLE<(i%TAILLE) && ( grille->tab[i-1] == LIBRE || grille->tab[i-1] ==INCONNU) ){
        n ++;
    }
    if((i+1)%TAILLE>(i%TAILLE) && ( grille->tab[i+1] == LIBRE || grille->tab[i+1] ==INCONNU) ){
        n ++;
    }
    if(i+TAILLE<TAILLE * TAILLE && ( grille->tab[i+TAILLE] == LIBRE || grille->tab[i+TAILLE] ==INCONNU) ){
        n ++;
    }
    return n;
}
...
int nbLampesPossiblesEtLibres(Grille *grille,int i){
    int n = 0;
    if(i-TAILLE>=0 && ( grille->tab[i-TAILLE] == LIBRE || grille->tab[i-TAILLE] ==INCONNU) && lampe_possible(i-TAILLE,grille) ){
        n ++;
    }
    if((i-1)%TAILLE<(i%TAILLE) && (i-1)>0 && ( grille->tab[i-1] == LIBRE || grille->tab[i-1] ==INCONNU) && lampe_possible(i-1,grille)){
        n ++;
    }
    if((i+1)%TAILLE>(i%TAILLE) &&  (i+1)<TAILLE*TAILLE && ( grille->tab[i+1] == LIBRE || grille->tab[i+1] ==INCONNU) && lampe_possible(i+1,grille)){
        n ++;
    }
    if(i+TAILLE<TAILLE * TAILLE && ( grille->tab[i+TAILLE] == LIBRE || grille->tab[i+TAILLE] ==INCONNU) && lampe_possible(i+TAILLE,grille)){
        n ++;
    }
    return n;
}
...
Case placerMur(Grille *grille,int i){
    ...
    if(nbCasesLibresAdj(grille,i)==4){
        ...
        switch(chiffreMur){
            case 0:
                if(nbCasesLibresAdj(grille,i)==4) grille->tab[i] = MUR0; else grille->tab[i] = MURV;
                break;
            case 1:
                grille->tab[i] = MUR1;
                break;
...
void ajouterLampe(Grille *grille, int casePossible[TAILLE],int longueur,int aPlacer){
    if(aPlacer > longueur){
    ...
    if(longueur != 0){
        ...
        for(int i = 0; i<aPlacer; i++){
            ...
            printf("( %d , %d ) -  ", casePossible[x]%TAILLE,casePossible[x]/TAILLE);
        }
        for(int j=0;j<longueur;j++){
            if(grille->tab[casePossible[j]] != LAMPE) grille->tab[casePossible[j]] = VIDE;
        }
    }
}
...
void placerLampe(Case actuel,int i, Grille *grille){
    ...
    int longueur = 0;
    if(actuel == MURV) return;
    if(actuel == MUR0){
        if(i-TAILLE>=0)grille->tab[i-TAILLE]=VIDE;
        if((i-1)%TAILLE<(i%TAILLE)) grille->tab[i-1]=VIDE;
        if((i+1)%TAILLE>(i%TAILLE)) grille->tab[i+1]=VIDE;
        if(i+TAILLE<TAILLE * TAILLE)grille->tab[i+TAILLE]=VIDE;
    }else{
        if(i-TAILLE>=0 ){
            if(lampe_possible(i-TAILLE,grille) && i-TAILLE>=0 ){
                ...
                if(grille->tab[i-TAILLE] == LIBRE || grille->tab[i-TAILLE] == INCONNU) grille->tab[i-TAILLE]=VIDE;
            }
        }
        if((i-1)%TAILLE<(i%TAILLE) ){
            if(lampe_possible(i-1,grille) &&  i-1>=0  ){
                ...
                if(grille->tab[i-1] == LIBRE || grille->tab[i-1] == INCONNU) grille->tab[i-1]=VIDE;
            }
        }
        if((i+1)%TAILLE>(i%TAILLE) ){
        ...
        if(actuel == MUR1) ajouterLampe(grille,casePossible,longueur,1);
        if(actuel == MUR2) ajouterLampe(grille,casePossible,longueur,2);
        if(actuel == MUR3) ajouterLampe(grille,casePossible,longueur,3);
        if(actuel == MUR4) ajouterLampe(grille,casePossible,longueur,4);
    }
    switch (grille->tab[i]){
        case MURV:
...
Grille *genere_grille(int pourcentMur){
    int x = 0;
    Grille *res = (Grille *) malloc(sizeof(Grille));
    int i = 0;
    Case actuel;
...
bool lampe_possible(int i,Grille *grille){
    ...
    if(grille->tab[i] != LIBRE && grille->tab[i] != INCONNU) return false;

    for(int j = 0;j<TAILLE;j++){
        if(grille->tab[ligne*TAILLE+j] == LAMPE && ligne*TAILLE+j <= i)
            lignePossible = false;
        if( (grille->tab[ligne*TAILLE+j] == MURV || grille->tab[ligne*TAILLE+j] == MUR0 || grille->tab[ligne*TAILLE+j] == MUR1 || grille->tab[ligne*TAILLE+j] == MUR2 || grille->tab[ligne*TAILLE+j] == MUR3 || grille->tab[ligne*TAILLE+j] == MUR4 ) && ligne*TAILLE+j < i)
            lignePossible = true;
        if(grille->tab[ligne*TAILLE+j] == LAMPE && ligne*TAILLE+j > i)
            return false;
        if( (grille->tab[ligne*TAILLE+j] == MURV || grille->tab[ligne*TAILLE+j] == MUR0 || grille->tab[ligne*TAILLE+j] == MUR1 || grille->tab[ligne*TAILLE+j] == MUR2 || grille->tab[ligne*TAILLE+j] == MUR3 || grille->tab[ligne*TAILLE+j] == MUR4 ) && ligne*TAILLE+j > i)
        ...
        if( (grille->tab[colonne+j*TAILLE] == MURV || grille->tab[colonne+j*TAILLE] == MUR0 || grille->tab[colonne+j*TAILLE] == MUR1 || grille->tab[colonne+j*TAILLE] == MUR2 || grille->tab[colonne+j*TAILLE] == MUR3 || grille->tab[colonne+j*TAILLE] == MUR4 ) && colonne+j*TAILLE<i)
